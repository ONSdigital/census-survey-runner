...
class TestLogin(IntegrationTestCase):
    ...
    def test_login_with_valid_token_should_redirect_to_survey(self):
        # Given
        token = self.token_generator.create_token('titles', 'test')
        ...
        # Then
        self.assertStatusOK()
        self.assertInUrl('/questionnaire/test/titles')
    ...
    def test_login_without_jti_in_token_is_unauthorised(self):
        # Given
        token = self.token_generator.create_token_without_jti('titles', 'test')
        self.get('/session?token=' + token)
    ...
    def test_login_without_case_id_in_token_is_unauthorised(self):
        # Given
        token = self.token_generator.create_token_without_case_id('titles', 'test')
        self.get('/session?token=' + token)
