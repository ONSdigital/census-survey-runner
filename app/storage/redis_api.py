...
def put(model, overwrite):  # pylint: disable=unused-argument
    ...
    if overwrite:
        get_db(config).set(key, ujson.dumps(item))  # pylint: disable=c-extension-no-member
    else:
        get_db(config).setnx(key, ujson.dumps(item))  # pylint: disable=c-extension-no-member
...
def get_by_key(model_type, key_value):
    ...
    if item:
        model, _ = schema.load(ujson.loads(item))  # pylint: disable=c-extension-no-member
        return model
